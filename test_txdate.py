...
if 'items' in data and 'charlie' in data['items'] and 'expenses' in data['items']['charlie']:
    ...
    if isinstance(expenses, list):
        for exp in expenses:
            if exp and exp.get('paymentMethod') == 'bpi_platinum' and 'txDate' not in exp:
                # Add a dummy date to test
                exp['txDate'] = '2026-08-01'
                count += 1
    else:
...
try:
    with urllib.request.urlopen(req_put) as response:
        ...
        print(f'Success! Updated {count} items with a default txDate.')
except Exception as e:
